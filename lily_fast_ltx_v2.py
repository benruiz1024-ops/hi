...
# NO bitsandbytes: it can crash on Kaggle P100 with "no kernel image available".
subprocess.run([
    sys.executable,'-m','pip','install','-q','-e','.[inference]',
    'gradio>=5,<7','imageio-ffmpeg','hf_xet'
], check=True)
...
# The XXL T5 stays on CPU. This avoids both GPU VRAM pressure and bitsandbytes CUDA kernels.
# FP16 cuts its resident RAM roughly in half. We only run it once per prompt.
...
for p in text_encoder_cpu.parameters():
    p.requires_grad_(False)

# IMPORTANT: text_encoder=None prevents the LTX pipeline from moving the huge T5 to CUDA.
...
def encode_prompt_cpu(prompt):
    # Use the same text cleanup as the official LTX pipeline.
    ...
    ids = toks.input_ids.to('cpu')
    mask_cpu = toks.attention_mask.to('cpu')
    start = time.perf_counter()
    with torch.inference_mode():
        emb_cpu = text_encoder_cpu(input_ids=ids, attention_mask=mask_cpu).last_hidden_state
    enc_time = time.perf_counter() - start
    emb = emb_cpu.to(device='cuda', dtype=DTYPE, non_blocking=False)
    mask = mask_cpu.to(device='cuda')
    del emb_cpu, ids, mask_cpu, toks
    gc.collect()
    return emb, mask, enc_time
...
def generate_video(prompt, image_path, preset, duration, seed):
    ...
    prompt_embeds, prompt_mask, enc_time = encode_prompt_cpu(prompt)
    # Distilled mode uses guidance_scale=1, so the negative branch is never selected.
    # Supply zero tensors only because the legacy pipeline asks for them during setup.
    neg_embeds = torch.zeros_like(prompt_embeds)
    neg_mask = torch.zeros_like(prompt_mask)
    ...
    del result, conditioning_items, prompt_embeds, prompt_mask, neg_embeds, neg_mask
    gc.collect(); torch.cuda.empty_cache()
    return str(out_path), (
        f'✅ {duration} • {width}×{height} • {len(timesteps)} steps • '
        f'prompt {enc_time:.1f}s + render {render_time:.1f}s • seed {seed} • {GPU_NAME}'
    )

def ui_generate(prompt,image,preset,duration,seed):
    try:
        return generate_video(prompt,image,preset,duration,seed)
    except torch.cuda.OutOfMemoryError:
        torch.cuda.empty_cache()
        raise gr.Error('GPU OOM — use TURBO 512×320.')
    except Exception as e:
        raise gr.Error(f'{type(e).__name__}: {e}')

with gr.Blocks(title='Lily Fast Video Studio V2') as demo:
    gr.Markdown('## ⚡ Lily Fast Video Studio V2\nP100/T4-safe build. T5 stays on CPU; LTX stays on GPU.')
    with gr.Row():
        with gr.Column():
